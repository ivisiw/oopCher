...
import { saveProject, loadProject, generateId } from '../lib/projectStorage';
import { shapeFromJSON } from '../lib/shapeFromJSON';
...
export default function Editor({ projectId }: { projectId: string }) {
  ...
  const projectIdRef = useRef(projectId === 'new' ? generateId() : projectId);
  ...
  useEffect(() => {
    ...
    loadProject(projectId).then(data => {
      ...
      const shapes = (data.shapes as Record<string, unknown>[])
        .map(s => shapeFromJSON(s as Record<string, unknown>))
        .filter((s): s is NonNullable<typeof s> => s !== null);
      shapes.forEach(shape => dispatch({ type: 'ADD_SHAPE', shape }));
    });
  }, [projectId]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      await saveProject(projectIdRef.current, projectName, stateRef.current.shapes, 'bresenham');
      setSaveStatus('saved');
    ...
    <div style={{ height: '100vh', display: 'flex', background: '#0f172a', color: '#e2e8f0', fontFamily: 'system-ui,sans-serif', overflow: 'hidden' }}>
      ...
      <div style={{ width: 144, background: '#1e293b', display: 'flex', flexDirection: 'column', gap: 6, padding: 12, borderRight: '1px solid #334155', flexShrink: 0 }}>
        <button onClick={() => navigate('/gallery')} style={{
          padding: '6px 0', background: 'transparent', color: '#64748b', border: '1px solid #334155',
          borderRadius: 6, cursor: 'pointer', fontSize: 12, marginBottom: 4,
        }}
        ...
        <button onClick={handleSave} style={{
          ...
          background: saveStatus === 'saved' ? '#16a34a' : saveStatus === 'error' ? '#dc2626' : '#3b82f6',
          ...
          {selectedShape ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
                {btn('✥ Move', () => dispatch({ type: 'SET_MODE', mode: 'moving' }), state.mode === 'moving')}
