...
test("rotate: 0 and 90 degrees (explicit expected values)", () => {
    ...
    const expected90: Mat3 = [Math.cos(angle), -Math.sin(angle), 0, Math.sin(angle),  Math.cos(angle), 0, 0, 0, 1];
...
test("multiply: black-box property using points (A*B acting like A after B)", () => {
    ...
    const points = [{x:0,y:0}, {x:1,y:0}, {x:0,y:1}, {x:2.3,y:-1.7}, {x:-4.1,y:3.2}];
